...
class PoseVelocityCommandCfg(CommandTermCfg):
    ...
    target_patch_keys: list[str] = ["target"]
    """Keys of the terrain flat patches to sample position targets from.

    If multiple keys are given, the flat patches of all the given keys are concatenated and a target is
    sampled uniformly from the combined set. This is useful to e.g. sample targets from both a generic
    "target" patch group and a terrain-specific patch group (e.g. "stair_steps").
    """
