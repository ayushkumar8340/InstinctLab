...
class PoseVelocityCommand(CommandTerm):
    ...
    def __init__(self, cfg: PoseVelocityCommandCfg, env: ManagerBasedEnv):
        ...
        missing_patch_keys = [key for key in self.cfg.target_patch_keys if key not in self.terrain.flat_patches]
        if missing_patch_keys:
            raise RuntimeError(
                f"The terrain-based command generator requires valid flat patches under {missing_patch_keys} in the"
                f" terrain. Found: {list(self.terrain.flat_patches.keys())}"
            )
        # valid targets: (terrain_level, terrain_type, num_patches, 3)
        # if multiple patch keys are specified, concatenate the patches from all of them along the patches dim
        self.valid_targets: torch.Tensor = torch.cat(
            [self.terrain.flat_patches[key] for key in self.cfg.target_patch_keys], dim=2
        )
