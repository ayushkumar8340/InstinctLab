    ...
    sub_terrains={
        ...
        "pyramid_stairs": terrain_gen.PerlinPyramidStairsTerrainCfg(
            ...
            flat_patch_sampling={
                ...
                "stair_steps": FlatPatchSamplingCfg(
                    num_patches=30,
                    patch_radius=[0.03, 0.05, 0.08],
                    max_height_diff=0.05,
                    x_range=(-3.0, 3.0),
                    y_range=(-3.0, 3.0),
                ),
...
# Flat ground sub-terrain shared by the stairs+flat terrain configs below.
_STAIRS_FLAT_FLAT_TERRAIN_CFG = terrain_gen.PerlinPlaneTerrainCfg(
    proportion=0.3,
    noise_scale=0.0,
    noise_frequency=20,
    fractal_octaves=2,
    ...
    flat_patch_sampling={
        ...
        "stair_steps": FlatPatchSamplingCfg(
            num_patches=30, patch_radius=[0.05, 0.10, 0.15, 0.20], max_height_diff=0.05
        ),
    },
)

# Stairs-only sub-terrains, reused (deep-copied) from the rough terrain config above. They already define both
# the "target" (top platform) and "stair_steps" (multiple points spread over the staircase) flat patches.
_STAIRS_FLAT_SUB_TERRAINS = {
    "flat": _STAIRS_FLAT_FLAT_TERRAIN_CFG,
    "pyramid_stairs": copy.deepcopy(ROUGH_TERRAINS_CFG.sub_terrains["pyramid_stairs"]),
    "pyramid_stairs_high": copy.deepcopy(ROUGH_TERRAINS_CFG.sub_terrains["pyramid_stairs_high"]),
    "pyramid_stairs_inv": copy.deepcopy(ROUGH_TERRAINS_CFG.sub_terrains["pyramid_stairs_inv"]),
    "pyramid_stairs_inv_high": copy.deepcopy(ROUGH_TERRAINS_CFG.sub_terrains["pyramid_stairs_inv_high"]),
}
# re-normalize the proportions so that they sum to 1
...
STAIRS_FLAT_TERRAINS_CFG = TerrainGeneratorCfg(
    ...
    use_cache=False,
    curriculum=True,
    sub_terrains=copy.deepcopy(_STAIRS_FLAT_SUB_TERRAINS),
)
"""Terrain made up of only stairs (regular and inverted, low and high) and flat ground."""

SMALL_STAIRS_FLAT_TERRAINS_CFG = TerrainGeneratorCfg(
    seed=0,
    size=(8.0, 8.0),
    border_width=3,
    num_rows=2,
...
"""Same sub-terrains as :data:`STAIRS_FLAT_TERRAINS_CFG` but with a much smaller grid, intended for debugging on
...
class CommandsCfg:
    ...
    base_velocity = mdp.PoseVelocityCommandCfg(
        asset_name="robot",
        # reduce this if something goes wrong
