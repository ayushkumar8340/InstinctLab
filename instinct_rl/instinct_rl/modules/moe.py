...
class MoeLayer(nn.Module):
    def __init__(
        ...
        # running mean of the gate scores over the last forward pass, used to log which experts are
        # being used (e.g. to detect dead/unused experts).
        self.register_buffer("last_gate_scores", torch.ones(num_experts) / num_experts)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            self.last_gate_scores.copy_(gate_scores.mean(dim=0))
        return output
